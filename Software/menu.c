...
static void lcd_print_line(uint8_t line, const char *s)
{
    char buf[21];
    size_t i;
    for (i = 0; i < 20 && s && s[i]; ++i) buf[i] = s[i];
    for (; i < 20; ++i) buf[i] = ' ';
    buf[20] = '\0';
    ...
    if (delta) {
        ...
        if (menu.active) {
            if (menu.editing != EDIT_NONE) {
                ...
                uint8_t max = 1;
                switch (menu.editing) {
                    case EDIT_VREF: max = 2; break;
                    case EDIT_GPS_BAUD: max = 8; break;
                    case EDIT_STOPBITS: max = 3; break;
                    case EDIT_PARITY: max = 3; break;
                }
