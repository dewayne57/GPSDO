...
void _i2cSclHigh(void) {
    TRISC |= SCL; // Release SCL (open drain - pull up will take it high)
    I2C_HALF_DELAY();
    __delay_us(2); // Additional delay for line to stabilize
}
...
void _i2cSdaHigh(void) {
    TRISC |= SDA; // Release SDA (open drain - pull up will take it high)
    I2C_HALF_DELAY();
    __delay_us(2); // Additional delay for line to stabilize
}
