...
void gps_init(void)
{
    ...
    U1BRGH = (uint8_t)((baud_div >> 8) & 0xFF);
...
void gps_format_date(char *buffer, const gps_datetime_t *dt)
{
    ...
    {
        sprintf(buffer, "%02d-%02d-%02d", dt->day, dt->month, dt->year);
    }
    else
    {
        strcpy(buffer, "--/--/--");
